...
function runningMap() {
  ...
  d3.csv(config.source).then(function (data) {
    data = data.map(function (d) {
      return [+d.lon, +d.lat, +d.index, +d.len];
    });
    ...
    var tracks = dataContainer
      .selectAll("custom.geoPath")
      .data(nested)
      .enter()
      .append("custom")
      .classed("geoPath", true);

    var runners = dataContainer
    ...
    function drawCanvas() {
      ...
      tracks.each(function () {
        ...
        if (t > 0 && t < trackData.length) {
          ...
          path({
            type: "LineString",
            coordinates: [trackData[t - 1], trackData[t]],
          });
      ...
      runners.each(function () {
        var node = d3.select(this);
        contextPoints.moveTo(
          parseFloat(node.attr("x")) + parseFloat(config.radius),
          node.attr("y")
        );
        contextPoints.arc(
          node.attr("x") + config.radius,
          node.attr("y"),
          config.radius,
          0,
          2 * Math.PI
    ...
    function step(t) {
      runners
        .attr("x", function (d) {
          return coord_slicer(d, t)[0];
        })
        .attr("y", function (d) {
          return coord_slicer(d, t)[1];
        });
